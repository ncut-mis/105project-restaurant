...
@section('content')
    ...
    如果廚房無法正常顯示櫃台傳送的通知，請點擊下方更新紐，系統將自動調整。
@foreach($token as $tk)
    <form action="{{route('backstage.chef.notify.update',$tk->id)}}" method="POST" role="form">
        {{ csrf_field() }}
        {{ method_field('PATCH') }}
        <p id="token" hidden ></p>
        <script>
        ...
        <div>
            <input name="token2" class="form-control" placeholder="請輸入下方顯示之個人驗證碼" id="abc" type="hidden">
        </div>
